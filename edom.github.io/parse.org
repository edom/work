...
If the parser is a function \(P : C^* \to T\), then the grammar is /unambiguous/.

A /lexer/ is a degenerate[fn::https://en.wikipedia.org/wiki/Degeneracy_(mathematics)] parser: it is a parser whose codomain is a list (which is a degenerate tree).
