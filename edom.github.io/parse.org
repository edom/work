#+TITLE: On parsing formal languages
...
* On the set-theoretic fixed-point semantics of context-free non-terminals
<2019-09-05>

Notation: Let \(X \cdot Y = \SetBuilder{xy}{x \in X, ~ y \in Y}\).
We call this the "concatenating product".
...
Hunch:
We can use that to make a parser, because a non-terminal rule can be thought of as a set endofunction like that.

For example, consider the rule \( R : A \to A b | c \).
The rule can be thought as the function \( \hat{R}(\hat{A}) = (\hat{A} \cdot \Set{b}) \cup \Set{c} \).
The non-terminal \(A\) can be thought of as representing the set \( \hat{A} \)
satisfying the fixed-point equation \( \hat{A} = (\hat{A} \cdot \Set{b}) \cup \Set{c} \)
