...
This is a summary of https://plato.stanford.edu/entries/geometry-finitism/.
...
Hodons.
** What is fundamental in geometry?
Point? Line? Adjacency? Distance?

For every point P, there are \(n\) points \(Q_1,\ldots,Q_n\) such that \(d(P,Q_k) = 1\).
** Synthetic finite geometry via discrete topology and graph theory
We assume a global parameter \(n\) (neighborhood granularity).
...
For example, we can quantize two-dimensional Euclidean geometry with 100 unit vectors (akin to dividing a circle into 100 sectors).
...
If \( X + \vec{v} = Y \) then \( X = Y + (-\vec{v}) \).
If \(Y\) is the \(\vec{v}\)-ward neighbor of \(X\), then \(X\) is the reverse-\(\vec{v}\)-ward neighbor of \(Y\).
