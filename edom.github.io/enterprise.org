...
I think the job of the programmer is to formalize the business user's implicit ontology
...
We are programmers ourselves, so we already know a programmer's mental model, so we need not talk about it further,
but I have never thought about the end-user's mental model until now.
Matching mental model is a requirement of good user experience.

The user's mental model is imperative/procedural.

The users think of a software system as the set of things that the user can do with the system.
The users think of a software system as the set of the ways they can interact with the system.
...
#+END_EXAMPLE
* What mess?
** Ontology engineering by competency questions
...
For example, an accounting ontology should be able to answer questions like:
...
The end goal of an accounting ontology is for reporting, auditing, and integration.
