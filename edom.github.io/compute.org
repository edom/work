...
What do we mean when we say "This machine computes \(f(x)\) given \(x\)"?
What do we mean by "This calculator can compute square roots"?
What do we mean by "This machine computes the sum of two numbers"?

We can analyze the transitive verb "to compute" with these two questions:
What computes?
What is computed?

These may be computed:
a value, a set, a relation, a function, a number, and a sequence.

A machine "computes the value \(y\) from the expression \(x\)"
iff the machine starts with a representation of \(x\) and ends with a representation of \(y\).
The key concepts are start, end, and representation.
For example, a machine may compute \(x+y\) from \(x\) and \(y\).

A machine "computes the set \(D\)" iff, for each \(x \in D\), the machine determines the truth of \(r(x) \in R(D)\),
...
A machine "computes the sequence \(x\)" iff, given ever-longer time to run, the machine computes an ever-longer prefix of the sequence.
