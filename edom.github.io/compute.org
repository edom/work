...
* What is computation?
** The plan
We want to define the verb "compute".
First we analyze the English suffixes "-er" and "-ation".
Then we present a short history of computers.
Then we reconcile analog computers and digital computers.
Then we analyze the subject and object of the verb "compute".
Finally we extract the essence: we find a definition of "compute" that satisfies all those computers.
** Computer, computation, and the meaning of English suffixes
We understand a word by scrutinizing its /origin/ (where it comes from) and /usage/ (how it is used).

"Computer" comes from "compute" and "-er".

"Computation" comes from "compute" and "-ation".

Appending -er to a verb V forms the noun Ver which means "one who does V", which we often stretch to mean "a thing that does V".
This suffix works for /all/ verbs,
including both Germanic and Latinate verbs, and even neologisms.
For example, a beer (be-er) is "one who is (exists)", and a forebear (forebeer) is one who exists earlier/before[fn::https://en.wiktionary.org/wiki/beer#Noun_2].
Once we accept that V is a verb, we readily accept that Ver is a noun that means one who does V.
After we have accepted that "google" is a verb, we readily accept that a "googler" is someone who "googles".
...
This is a fundamental assumption we make about the Universe?
But this has a problem: if we assume that, then the sentence "I exist" implies that I cause my own existence, which is somewhat problematic,
because it seems problematic for something to cause itself[fn::https://en.wikipedia.org/wiki/Causa_sui]?
Did we invent God because we impose, through the syntax of our languages, that everything has a cause, that every verb implies that there is a subject that does it?
...
Appending -ion to a verb Vate forms the noun Vation.
But only Latinate verbs take -ion.
...
I think generally -re → -te → -tion.
Germanic verbs do not take -ion: eat (German essen, Dutch eten), sleep (German schlafen, Duth slapen).
But Latinate verbs do take -ing: manducating (Latin manducare), computing (Latin computare).
...
(That is not how those words were actually historically imported though.)
...
English is messy[fn::https://en.wikipedia.org/wiki/Doublet_(linguistics)].
