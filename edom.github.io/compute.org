...
An example usage is "this machine computes the sum of two numbers".

A machine /computes/ the expression \(f(x)\) from the value \(x\)
iff the machine starts with a representation of \(x\) and ends with a representation of the result of evaluating \(f(x)\).

A machine computes the function \(f:D\to C\) iff the machine computes \(f(x)\) from \(x\) for each \(x\in D\).
