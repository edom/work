...
This implies that /computation is a manipulation of non-material things/.

The subject of "compute" may have material existence:
both a concrete machine and an abstract computation model compute.
A machine simply acts according to the laws of physics, but we /interpret/ such acts as computation.

There are computers not designed by man.
We are computers designed by nature.
Some animals can count[fn::http://www.bbc.com/future/story/20121128-animals-that-can-count],
and counting is a computation, so they can compute.
Those animals would still compute even if there were no humans to describe the computation.
*** An attempt to define "to compute"
