...
data Query
    ...
    | Project [I.Col] Query -- ^ select some columns of subquery
    | QJoin Query Query
...
renderSqlSelect q = case q of
    From t ->
        -- TODO escape
        "SELECT " ++ L.intercalate ", " (map I.getName $ I.tCols t) ++ " FROM " ++ maybe "" (++ ".") (I._schema t) ++ I.tName t
    Project cols subq ->
        "SELECT " ++ L.intercalate ", " (map I.getName cols)
