...
(define (read-all source in)
    ;;  Quagmire: Why does using "read" cause source to be #f?
    (define (do-read in)
        (define syn (read-syntax source in))
        (if (eof-object? syn) eof (syntax->datum syn)))
    (for/list ((item (in-port do-read in))) item))

(parameterize ( (port-count-lines-enabled #t)
                (current-readtable my-readtable) )
    (define source "/home/erik/work/etc/emacs.d/elpa/org-plus-contrib-20180910/org.el")
    (call-with-input-file source (lambda (port)
        (pretty-print (read-all source port))
    )))
