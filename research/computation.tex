...
The \emph{final state} of \(D\) for input \(w \in A^*\) is \(f(w)\)
...
A \emph{machine} is \((S,I,f)\)
where \(S\) is the \emph{state set},
\(I \subseteq C\) is the \emph{initial state set},
and \(f \subseteq C \times C\) is the \emph{transition relation}.
